...
export default function Sidebar({ onLogExpense }) {
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[232px] flex-col border-r-[3px] border-ink bg-surface p-4 lg:flex">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-2.5 border-[3px] px-2.5 py-2 text-sm font-semibold transition-transform ${
                isActive
                  ? `${NAV_ACTIVE_BG[item.key]} border-ink text-on-accent shadow-hard-sm`
                  : 'border-transparent hover:border-ink hover:bg-surface-2'
              }`
            }
          >
      ...
      <button
        ...
        className="press flex w-full items-center justify-center gap-2 border-[3px] border-ink bg-invert py-3 font-display text-[13px] text-yellow shadow-[5px_5px_0_var(--color-pink)]"
